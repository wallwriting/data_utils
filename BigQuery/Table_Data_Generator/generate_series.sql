...
DECLARE rangeVar INT64 DEFAULT 3;
...
DECLARE multVar INT64 DEFAULT 3;
...
CREATE TABLE IF NOT EXISTS dpa.meta_high_sequence
...
BEGIN TRANSACTION;
  WHILE cursorVar < rangeVar
    DO
    INSERT INTO dpa.meta_series
    SELECT
        (SELECT high_sequence_number FROM dpa.meta_high_sequence)
        +
        CAST(ROW_NUMBER() OVER() AS INT64)
    FROM
      dpa.meta_series a1
    ...
    UPDATE
      dpa.meta_high_sequence
    SET
      high_sequence_number = (SELECT MAX(sequence_number) FROM dpa.meta_series)
    WHERE 1=1
    ;
